...
/**
 * A computational unit in a message-passing network. A {@link Node} receives a stream of messages
 * to process. Typically, these messages are sent from other {@link Node} instances, but may also
 * arrive from other sources. A {@link Node} is intended to be used in the context of "think like a
 * vertex" or actor-based modeling, where each {@link Node} maintains internal state and may run
 * concurrently with other {@link Node} instances.
 *
 ...
 * as when the {@link Node} will be removed from the network.
 *
